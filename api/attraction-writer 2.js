...
const instructionBlock = (input) => {
  ...
  const oneTimeInstruction = cleanText(input.custom_instruction);
  const rules = Array.isArray(input.custom_rules)
    ...
    : [];
  return `\nPROJECT WRITING INSTRUCTIONS (apply to research, copy, and revisions):\n${projectInstructions || 'None supplied.'}\n\nSAVED COPYWRITING RULES (mandatory unless they conflict with factual accuracy or safety):\n${rules.length ? rules.join('\n') : 'Use the built-in standards below.'}\n\nONE-TIME REQUEST FOR THIS ITEM:\n${oneTimeInstruction || 'None supplied.'}\n`;
};
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (i.action === 'refine-chat') {
      const prompt = `You are a senior attraction copy editor. Revise this attraction copy according to the manager request. Exact heading: "${heading}". Write exactly 3 or 4 plain paragraphs totaling 180-260 words; end every paragraph with <br><br>. No questions, bullets, we/our/let's, or these phrases: ${BANNED.join(', ')}. Use grounded facts, named creators/features when known, and end with the value and flexibility of a private local guide.${instructionBlock(i)}\nAttraction/location: ${name}, ${i.city || ''}, ${i.country || ''}\nCurrent copy: ${i.current_content || ''}\nManager request: ${i.user_prompt}\nResearch: ${JSON.stringify(i.research || {})}`;
